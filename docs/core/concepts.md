...
- **[Концепты ядра](concepts/core.md)** - TASK, WORKFLOW, AGENT, CONTEXT, EVENT
...
│  - Независим от домена                      │
│  - TASK, WORKFLOW, AGENT, CONTEXT, EVENT    │
└─────────────────────────────────────────────┘
                    ↑
                    │ использует
                    │
...
→ **[Подробнее](concepts/core.md#1-task)**
...
### 5. EVENT
Система событий для межкомпонентной коммуникации. Слабая связанность между пакетами.

**Методы**: `emit()`, `subscribe()`, `unsubscribe()`, `get_history()`, `list_subscriptions()`

→ **[Подробнее](concepts/core.md#5-event)**
...
| Workflow | 7 состояний |
| DSL | YAML-based |
| События | ✅ В ядре (basic-events) |
...
2. Реализуйте концепты ядра (TASK, WORKFLOW, AGENT, CONTEXT, EVENT)
